...
def create_dataframe(parse):
    """
    create_dataframe is called by register_graph to build a dataframe from Coinbase candlestick chart data
    """
    df = pd.DataFrame(
        parse,
        columns=[
...
def register_graph(app):
    """
    register_graph functionalizes chart callbacks into app.py
    """
    ...
    def update_output(product_selection, granularity_selection):
        """
        update_output updates Plotly candlestick chart on UI product and granularity inputs
        """
